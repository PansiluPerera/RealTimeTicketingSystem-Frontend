...
function Register() {
  const { role } = useParams();  // Get role from URL (customer or vendor)
  const navigate = useNavigate();
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [vendorName, setVendorName] = useState(''); // For vendor registration
  const [vendorUserName, setVendorUserName] = useState(''); // For vendor registration
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post(registerEndpoint, formData, {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded', // This header is for form data
        },
      });
